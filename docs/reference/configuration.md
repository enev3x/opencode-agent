...
- [Task System](#task-system)
  ...
  - [Odin Agent](#odin-agent)
  - [Odin Tasks](#odin-tasks)
- [Features](#features)
  - [Skills](#skills)
  - [Hooks](#hooks)
  - [Commands](#commands)
...
```jsonc
{
  ...
  "agents": {
    ...
    // Mimir inherits odin model; just add prompt guidance
    "mimir": {
...
Override built-in agent settings. Available agents: `odin`, `thor`, `mimir`, `volva`, `bragi`, `explore`, `huginn`, `urd`, `forseti`, `heimdall`, `einherjar`.
...
Disable agents entirely: `{ "disabled_agents": ["volva", "huginn"] }`

Agent tab cycling defaults to Odin, Thor, Mimir, Heimdall. Override known agent ordering with `agent_order`; omitted core agents keep their default relative order. Unknown or duplicate names are ignored and reported with a config toast.
...
Mimir is the exception for prompt replacement: its mandatory planner prompt always remains active so it can load `ulw-plan` first. For `agents.mimir`, both `prompt` and `prompt_append` are appended to the mandatory base prompt instead of replacing it.
...
{
  "agents": {
    "volva": { "thinking": { "type": "enabled", "budgetTokens": 200000 } }
  }
}
```
...
For Mimir, file-backed `prompt` content is appended after the mandatory base prompt; it does not replace the base prompt.
...
Domain-specific model delegation used by the `task()` tool. When Odin delegates work, it picks a category, not a model name.
...
| **Odin Junior** | `claude-sonnet-4-6` | `anthropic\|github-copilot\|opencode\|vercel/claude-sonnet-4-6` → `opencode-go\|vercel/kimi-k3` → `openai\|github-copilot\|opencode\|vercel/gpt-5.6-sol (medium)` → `opencode-go\|vercel/minimax-m3` → `minimax-coding-plan\|minimax-cn-coding-plan/MiniMax-M3` → `opencode-go\|vercel/minimax-m2.7` → `opencode/big-pickle` |
...
| Option                    | Default | Description                                                     |
| ------------------------- | ------- | --------------------------------------------------------------- |
| `disabled`                | `false` | Disable all Odin orchestration, restore original build/plan |
| `default_builder_enabled` | `false` | Enable OpenCode-Builder agent (off by default)                  |
| `planner_enabled`         | `true`  | Enable Mimir (Planner) agent                               |
| `replace_plan`            | `true`  | Demote default plan agent to subagent mode                      |

Odin agents can also be customized under `agents` using their names: `Odin`, `OpenCode-Builder`, `Mimir (Planner)`, `Urd (Plan Consultant)`.
...
The `odin.tasks` section configures **storage options** only:
...
| `task_system`                            | `false`    | Enable Odin task system                                                          |
...
Native Windows Codex installs bundle a `git_bash` MCP server and write `[plugins."omo@odinlabs".mcp_servers.git_bash] enabled = true`. Non-Windows installs keep the bundled manifest entry but write `enabled = false`, so the plugin detail can still show the server while policy prevents exposure.
...
`lazycodex doctor` warns when `omo@odinlabs` is enabled and the companion plugin is enabled, or when stale `[hooks.state."codex@openai-codex:..."]` SessionStart/Stop trust entries remain in `~/.codex/config.toml`. The doctor only reports this condition; it does not disable or delete another plugin for you.
