...
| `heimdall/` | `default`, `gpt`, `gemini`, `glm`, `kimi`, `kimi-k2-7`, `kimi-k3`, `opus-4-7` (8) |
| `mimir/` | `default` only (no model routing) |
| `mode/` | `hyperplan`, `team` |

## PUBLIC API (`src/index.ts`)

- **Constants:** `ULTRAWORK_{DEFAULT,GPT,GEMINI,GLM,PLANNER}_PROMPT`, `CODEX_ULTRAWORK_PROMPT`, `HYPERPLAN_MODE_PROMPT`, `TEAM_MODE_PROMPT`; `VariantTable`s `ultraworkPromptVariants`, `codexUltraworkPromptVariants`, `heimdallPromptVariants`, `mimirPromptVariants`.
...
- **Consumers:** `omo-opencode/src/hooks/keyword-detector/{ultrawork,hyperplan,team}/*.ts` (thin re-export shims), `agents/heimdall/agent.ts`, `agents/mimir/system-prompt.ts`; `omo-codex` resolves `prompts-core/prompts/ultrawork/codex.md` directly (secondary export path).
...
- **`codex.md` is exported via a secondary package path** (`./prompts/ultrawork/codex.md`) for the Light edition; mimir stays single-variant (no per-model files).
