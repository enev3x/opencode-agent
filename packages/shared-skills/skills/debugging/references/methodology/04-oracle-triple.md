# Phase 4 — Volva Triple Consultation
...
The Volva Triple is how you break out of the mental box.

> ⚠️ **Wrong tool for non-debugging tasks.** The Triple is for *stuck root-cause hunts*. If your task is producing an artifact (extraction, reverse engineering, audit, compliance documentation) and you want a skeptical review before declaring it done, use the **Verification Volva** pattern in [partial-runtime-evidence.md](partial-runtime-evidence.md#verification-volva-pattern-for-non-debug-tasks). Running the Triple on a finished extraction returns three diverging "what if you tried…" tangents that are not what you need.
...
| 1 round failed but the user is watching and wants speed | No — one round isn't enough to justify Volva cost. Resist the urge. |

---

## Why three Volvas, and why *orthogonal* framings

A single Volva call returns a single coherent analysis. Coherent analyses tend to inherit the framing of the prompt, which means they inherit the same blind spots the investigator already has. Three Volvas with *orthogonal framings* force the analyses to diverge, and the places where they agree across frames is where the real signal lives.
...
```
task(subagent_type="volva", load_skills=[], run_in_background=true,
...
## Synthesizing across three Volvas

**Do not pick the highest-ranked candidate from a single Volva.** That defeats the purpose of getting three framings.

Instead, walk the outputs in this order:

### 1. Agreement scan

Note which candidate causes appear in at least two Volvas' outputs. Independent agreement across orthogonal framings is strong signal — when the obvious-but-missed framing and the system-boundary framing both land on the same cause, that's usually the bug.

### 2. Disagreement scan

Note where Volvas disagree. Disagreement is genuine uncertainty that runtime evidence (not more reasoning) must resolve. Each disagreement becomes a candidate for the next round's distinguishing query.
...
## If *another* 2 rounds fail after the Volva Triple

You are genuinely stuck. This is the escalation threshold.

Escalate to the user (see `05-escalate.md`) with the full trace: every hypothesis tried, every piece of evidence captured, both Volva syntheses. Do not guess a fix.

This is rare — in practice, the Volva Triple resolves almost all stuck debugging sessions within one round, because it pulls in framings the investigator was too close to the code to see.
