...
describe("createTeamService curated agent gating", () => {
  test("#given a team member spec naming a curated read-only agent #when team_create validates members #then the curated rejection message surfaces", async () => {
    ...
    expect(Object.keys(engine.agents)).toContain("volva")
    const service = createTeamService({
      manager: engine.manager,
      runtime: engine.runtime,
      settings: engine.settings,
    ...
    await expect(
      service.createTeam({
        inlineSpec: {
          name: "curated-team",
          members: [{ name: "volva", kind: "subagent_type", subagent_type: "volva", prompt: "review the plan" }],
        },
      }),
    ).rejects.toThrow('curated read-only agent "volva" cannot be a team member; delegate via the task tool instead')
  })
})
