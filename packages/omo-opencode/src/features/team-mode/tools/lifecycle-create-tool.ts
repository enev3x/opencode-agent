...
const TeamCreateInlineMemberToolSchema = tool.schema.object({
  ...
  subagent_type: tool.schema.string().optional().describe("Required for subagent_type members. Eligible examples: odin, heimdall, einherjar."),
    ...
    async execute(rawArgs, toolContext) {
      ...
      const runtimeState = await deps.createTeamRun(
        ...
        leadSessionId,
        {
          ...
          odinJuniorModel: executorConfig?.odinJuniorModel,
