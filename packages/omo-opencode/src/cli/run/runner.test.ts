...
describe("resolveRunAgent", () => {
  ...
  it("uses CLI agent over env and config", () => {
    // given
    const config = createConfig({ default_run_agent: "mimir" })
    const env = { OPENCODE_DEFAULT_AGENT: "Heimdall" }

    // when
    const agent = resolveRunAgent(
      { message: "test", agent: "Thor" },
      config,
      env
    )

    // then
    expect(agent).toBe("thor")
  })
  ...
  it("uses config agent over default", () => {
    ...
    // then
    expect(agent).toBe("mimir")
  })

  it("falls back to odin when none set", () => {
    // given
  ...
  it("skips disabled odin for next available core agent", () => {
    // given
    const config = createConfig({ disabled_agents: ["odin"] })
  ...
  it("maps display-name style default_run_agent values to canonical prompt agent ids", () => {
    // given
    const config = createConfig({ default_run_agent: "Odin - Ultraworker" })
