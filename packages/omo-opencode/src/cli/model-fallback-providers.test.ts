...
describe("generateModelConfig provider routes", () => {
  describe("Thor agent special cases", () => {
    test("Thor is created when OpenAI is available (openai provider connected)", () => {
      ...
      // then Thor uses the native Sol route
      expect(result.agents?.thor?.model).toBe("openai/gpt-5.6-sol")
      expect(result.agents?.thor?.variant).toBe("medium")
    })

    test("Thor uses its merged Copilot GPT-5.6 Sol medium rung", () => {
      ...
      // then Thor uses the supported Copilot effort
      expect(result.agents?.thor).toEqual({
        model: "github-copilot/gpt-5.6-sol",
        variant: "medium",
      })
    })

    test("Thor is created when OpenCode Zen is available (opencode provider connected)", () => {
      ...
      // then Thor uses the OpenCode Sol route
      expect(result.agents?.thor?.model).toBe("opencode/gpt-5.6-sol")
      expect(result.agents?.thor?.variant).toBe("medium")
    })

    test("Thor is omitted when only Claude is available", () => {
      ...
      // then Thor has no eligible provider
      expect(result.agents?.thor).toBeUndefined()
    })

    test("Thor is omitted when only Gemini is available", () => {
    ...
    test("Thor is omitted when only ZAI is available", () => {
  ...
  describe("bragi agent special cases", () => {
    test("bragi uses Claude fallback when ZAI is available with Claude", () => {
      ...
      // then Bragi uses the current Claude fallback
      expect(result.agents?.bragi?.model).toBe("anthropic/claude-haiku-4-5")
      expect(JSON.stringify(result)).not.toContain("zai-coding-plan/glm-4.7")
    })

    test("bragi uses Claude fallback when Claude is available", () => {
      // given only Claude is available
      const config = createConfig({ hasClaude: true })
      ...
      // then Bragi uses the shared Claude fallback
      expect(result.agents?.bragi?.model).toBe("anthropic/claude-haiku-4-5")
    })
  })
  ...
  describe("special-case agents include fallback_models", () => {
    ...
    test("bragi includes fallback_models when OpenAI and opencode-go are both available", () => {
      ...
      // then Bragi includes its remaining fallbacks
      expect(result.agents?.bragi?.model).toBe("openai/gpt-5.4-mini-fast")
      expect(result.agents?.bragi?.fallback_models).toBeDefined()
      expect(result.agents?.bragi?.fallback_models?.length).toBeGreaterThan(0)
    })

    test("bragi is omitted when only ZAI is available", () => {
      // given only ZAI is available
      const config = createConfig({ hasZaiCodingPlan: true })
      ...
      // then Bragi has no current compatible route
  ...
  describe("Vercel AI Gateway provider", () => {
    ...
    test("bragi uses gateway minimax when only gateway is available", () => {
      // given only Vercel AI Gateway is available
      const config = createConfig({ hasVercelAiGateway: true })
      ...
      // then Bragi uses the gateway Minimax route
      expect(result.agents?.bragi?.model).toBe("vercel/minimax/minimax-m2.7-highspeed")
    })

    test("Thor uses gateway GPT-5.6 Sol when only gateway is available", () => {
      // given only Vercel AI Gateway is available
      const config = createConfig({ hasVercelAiGateway: true })
      ...
      // then Thor uses gateway GPT-5.6 Sol
      expect(result.agents?.thor?.model).toBe("vercel/openai/gpt-5.6-sol")
    })
  ...
  describe("MiniMax Coding Plan providers", () => {
    test("uses minimax.io MiniMax-M3 when only MiniMax Coding Plan is available", () => {
      ...
      expect(result.agents?.bragi?.model).toBe("minimax-coding-plan/MiniMax-M3")
      expect(result.agents?.explore?.model).toBe("minimax-coding-plan/MiniMax-M3")
      expect(result.agents?.heimdall?.model).toBe("minimax-coding-plan/MiniMax-M3")
      expect(result.agents?.["einherjar"]?.model).toBe("minimax-coding-plan/MiniMax-M3")
    ...
    test("keeps opencode-go MiniMax M3 ahead of Coding Plan fallback when both are available", () => {
      ...
      expect(result.agents?.heimdall?.model).toBe("opencode-go/kimi-k3")
      expect(result.agents?.heimdall?.fallback_models?.[0]?.model).toBe("opencode-go/minimax-m3")
      expect(result.agents?.heimdall?.fallback_models?.[1]?.model).toBe("minimax-coding-plan/MiniMax-M3")
      expect(result.agents?.heimdall?.fallback_models?.[2]?.model).toBe("opencode-go/minimax-m2.7")
    })
    ...
    test("uses minimaxi.com MiniMax-M3 when only MiniMax CN Coding Plan is available", () => {
      ...
      expect(result.agents?.bragi?.model).toBe("minimax-cn-coding-plan/MiniMax-M3")
