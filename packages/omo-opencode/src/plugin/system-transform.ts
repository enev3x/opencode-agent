...
import { reconcileOdinRuntimePrompt } from "../agents/odin-runtime-prompt-reconciler"
...
export function createSystemTransformHandler(
  ...
  return async (input, output): Promise<void> => {
    // The Odin prompt body is model-family-specific and baked at registration
    ...
    reconcileOdinRuntimePrompt(output.system, input.model?.id)
    ...
    const ultraworkMessage = getUltraworkMessage("odin", modelID)
