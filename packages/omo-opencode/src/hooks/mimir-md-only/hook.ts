...
import { isMimirAgent } from "./agent-matcher"
import { isAllowedFile } from "./path-policy"

const TASK_TOOLS = ["task", "call_omo_agent"]

export function createMimirMdOnlyHook(ctx: PluginInput) {
  return {
    "tool.execute.before": async (
      ...
      if (!isMimirAgent(agentName)) {
        return
      }

      const toolName = input.tool

      // Inject planning-only warning for task tools called by Mimir
      ...
      if (!filePath) {
       ...
       if (!isAllowedFile(filePath, ctx.directory)) {
         log(`[${HOOK_NAME}] Blocked: Mimir can only write to .omo/*.md`, {
           sessionID: input.sessionID,
           tool: toolName,
           filePath,
           agent: agentName,
         ...
         throw new Error(
           `[${HOOK_NAME}] Mimir is a planning agent. File operations restricted to .omo/*.md plan files only. ` +
