...
const { createMimirMdOnlyHook } = await import("./index")
const { MESSAGE_STORAGE } = await import("../../features/hook-message-injector")

describe("mimir-md-only", () => {
  const TEST_SESSION_ID = "ses_test_mimir"
  ...
  describe("agent name matching", () => {
    test("should enforce md-only restriction for exact mimir agent name", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "mimir")
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should enforce md-only restriction for Mimir display name Plan Builder", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "Mimir - Plan Builder")
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
        sessionID: TEST_SESSION_ID,
    ...
    test("should enforce md-only restriction for Mimir display name Planner", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "Mimir - Plan Builder")
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
    ...
    test("should not enforce restriction for non-Mimir agent", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "odin")
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
        sessionID: TEST_SESSION_ID,
   ...
   describe("with Mimir agent in message storage", () => {
     beforeEach(() => {
       setupMessageStorage(TEST_SESSION_ID, "mimir")
     })

    test("should block Mimir from writing non-.md files", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should allow Mimir to write .md files inside .omo/", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should inject workflow reminder when Mimir writes to .omo/plans/", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should block Mimir from writing .md files outside .omo/", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should block Mimir from writing .md files when .omo is only part of a path segment", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should block Mimir from writing .md files under .omo-backup", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should allow bash commands from Mimir", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "bash",
    ...
    test("should inject planning warning when Mimir calls task", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "task",
    ...
    test("should inject planning warning when Mimir calls task (research)", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "task",
    ...
    test("should inject planning warning when Mimir calls call_omo_agent", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "call_omo_agent",
  ...
  describe("with non-Mimir agent in message storage", () => {
    beforeEach(() => {
      setupMessageStorage(TEST_SESSION_ID, "odin")
    })

    test("should not affect non-Mimir agents", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
    ...
    test("should not inject warning for non-Mimir agents calling task", async () => {
      // given
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "task",
  ...
  describe("boulder state priority over message files (fixes #927)", () => {
    ...
    //#given session was started with mimir (first message), but /start-work set boulder agent to heimdall
    //#when user types "continue" after interruption (memory cleared, falls back to message files)
    //#then should use boulder state agent (heimdall), not message file agent (mimir)
    test("should prioritize boulder agent over message file agent", async () => {
      setupMessageStorage(TEST_SESSION_ID, undefined)
      
      // given - heimdall in boulder state (from /start-work)
      ...
      const hook = createMimirMdOnlyHook({
        client: {},
      ...
      // when / then - should NOT block because boulder says heimdall, not mimir
      await expect(
        hook["tool.execute.before"](input, output)
      ).resolves.toBeUndefined()
    })

    test("should use mimir from boulder state when set", async () => {
      // given - heimdall in message files (from some other agent)
      setupMessageStorage(TEST_SESSION_ID, "heimdall", { useSessionAgent: false })
      
      // given - mimir in boulder state (edge case, but should honor it)
      writeFileSync(BOULDER_FILE, JSON.stringify({
        active_plan: "/test/plan.md",
        started_at: new Date().toISOString(),
        session_ids: [TEST_SESSION_ID],
      ...
      // when / then - should block because boulder says mimir
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("File operations restricted to .omo/*.md plan files only")
    })
    ...
    test("should fall back to message files when session not in boulder", async () => {
      ...
      // when / then - should block because falls back to message files (mimir)
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("File operations restricted to .omo/*.md plan files only")
    })
  ...
  describe("blocked-write guidance message", () => {
    test("#given mimir writes product code #when the hook blocks #then the error routes to plan todos, never to subagent implementation", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "mimir")
      const hook = createMimirMdOnlyHook(createMockPluginInput())
      const input = {
