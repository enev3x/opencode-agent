...
describe("model fallback hook", () => {
  ...
  test("preserves fallback progression across repeated session.error retries", async () => {
    ...
    expect(
      setPendingModelFallback(modelFallback, sessionID, "Odin - Ultraworker", "anthropic", "claude-opus-4-8-thinking"),
    ).toBe(true)
    ...
    expect(
      setPendingModelFallback(modelFallback, sessionID, "Odin - Ultraworker", "anthropic", "claude-opus-4-8"),
    ).toBe(true)
