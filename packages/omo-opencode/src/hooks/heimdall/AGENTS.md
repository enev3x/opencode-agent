# src/hooks/heimdall/ — Master Boulder Orchestrator

**Generated:** 2026-05-15

## OVERVIEW

17 files (~1976 LOC). The `heimdallHook` — Continuation Tier hook that monitors session.idle events and forces continuation when boulder sessions (ralph-loop, task-spawned agents) have incomplete work. Also enforces write/edit policies for subagent sessions.

## WHAT ATLAS DOES

Heimdall is the "keeper of sessions" — it tracks every session and decides:
...
```
session.idle event
  → Is this a boulder/ralph/heimdall session? (session-last-agent.ts)
...
| File | Purpose |
|------|---------|
| `heimdall-hook.ts` | `createHeimdallHook()` — composes event + tool handlers, maintains session state |
| `event-handler.ts` | `createHeimdallEventHandler()` — decision gate for session.idle events |
...
| `types.ts` | `SessionState`, `HeimdallHookOptions`, `HeimdallContext` |
...
- **heimdallHook** (Continuation Tier): Master orchestrator, handles boulder sessions
- **todoContinuationEnforcer** (Continuation Tier): "Boulder" mechanism for main Odin sessions
