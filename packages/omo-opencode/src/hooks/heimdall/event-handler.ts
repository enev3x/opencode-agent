...
import type { HeimdallHookOptions, SessionState } from "./types"

export function createHeimdallEventHandler(input: {
  ctx: PluginInput
  options?: HeimdallHookOptions
  ...
  return async ({ event }): Promise<void> => {
    ...
    if (event.type === "session.error") {
      ...
      if (!isAbort) {
        ...
        await handleHeimdallSessionIdle({ ctx, options, getState, sessionID })
        if (
