...
import { createHeimdallHook } from "./heimdall-hook"
...
// Force process isolation in CI runner (globalThis.setTimeout override conflicts with other heimdall tests)
...
describe("heimdall background task retry", () => {
  let testDir: string
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `heimdall-background-retry-${randomUUID()}`)
  ...
  test("#given background tasks are still running #when retry fires before they finish #then heimdall keeps retrying until continuation can resume", async () => {
    // given
    ...
    const hook = createHeimdallHook(unsafeTestValue<PluginInput>({
      directory: testDir,
      client: {
        session: {
  ...
  test("#given multiple idle events arrive while background retry is already pending #when tasks are still running #then heimdall keeps only one retry timer active", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
  ...
  test("#given background tasks keep running across multiple retries #when they finally finish on a later retry #then heimdall resumes exactly once", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
  ...
  test("#given retry gate sees no running task but injector still does #when retry fires #then heimdall schedules another retry and does not advance cooldown", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
  ...
  test("#given a persisted descendant becomes ineligible before retry fires #when retry runs #then heimdall re-checks descendant eligibility and does not inject", async () => {
    // given
    const descendantSessionID = "ses_descendant_retry_mismatch"
    setSessionAgent(descendantSessionID, "heimdall")
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
      active_plan: planPath,
  ...
  test("#given continuation injection is already in flight #when another idle event arrives #then heimdall does not inject twice", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
  ...
  test("#given a retry timer fires during an in-flight continuation that later fails #when the in-flight guard re-arms retry #then heimdall can recover on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
  ...
  test("#given a retry-driven continuation fails once #when retry handling re-arms the chain #then heimdall recovers on the next retry", async () => {
    // given
    const planPath = join(testDir, "test-plan.md")
    writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
    writeBoulderState(testDir, {
