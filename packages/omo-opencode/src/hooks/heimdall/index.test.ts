...
import type { HeimdallHookOptions, PendingTaskRef } from "./types"
import { createHeimdallHook } from "./index"
import { createToolExecuteAfterHandler } from "./tool-execute-after"
import { createToolExecuteBeforeHandler } from "./tool-execute-before"

const callerAgentBySession = new Map<string, string>()
type MockHeimdallInput = Parameters<typeof createHeimdallHook>[0] & {
  _promptMock: ReturnType<typeof mock>
  _sessionGetMock: ReturnType<typeof mock>
}

describe("heimdall hook", () => {
  let TEST_DIR: string
  let OMO_DIR: string
  ...
  function createMockPluginInput(overrides?: {
    ...
    return {
      directory: TEST_DIR,
      project: {} as Parameters<typeof createHeimdallHook>[0]["project"],
      worktree: TEST_DIR,
      experimental_workspace: { register: () => {} },
      serverUrl: new URL("http://localhost"),
      $: {} as Parameters<typeof createHeimdallHook>[0]["$"],
      client,
  ...
  function createTestHeimdallHook(
    input = createMockPluginInput(),
    options: Partial<HeimdallHookOptions> = {},
  ): ReturnType<typeof createHeimdallHook> {
    const resolvedOptions: HeimdallHookOptions = {
      directory: TEST_DIR,
      idleSettleMs: 0,
      isCallerOrchestrator: async (sessionID) => callerAgentBySession.get(sessionID ?? "") === "heimdall",
      ...options,
    }
    return createHeimdallHook(input, resolvedOptions)
  }
  ...
  beforeEach(() => {
    ...
    TEST_DIR = join(tmpdir(), `heimdall-test-${randomUUID()}`)
  ...
  describe("tool.execute.after handler", () => {
    test("should handle undefined output gracefully (issue #1035)", async () => {
      ...
      const hook = createTestHeimdallHook(createMockPluginInput())
    ...
    test("should ignore non-task tools", async () => {
     ...
     test("should not transform when caller is not Heimdall", async () => {
       // given - boulder state exists but caller agent in message storage is not Heimdall
     ...
     test("should append standalone verification when no boulder state but caller is Heimdall", async () => {
       // given - no boulder state, but caller is Heimdall
       const sessionID = "session-no-boulder-test"
       setupMessageStorage(sessionID, "heimdall")
     ...
     test("should transform output when caller is Heimdall with boulder state", async () => {
       // given - Heimdall caller with boulder state
    ...
    test("should preserve metadata when transforming output for boulder orchestrator", async () => {
      // given - Heimdall caller with boulder state and metadata containing sessionId
...
session_id: ses_subagent_abc
      ...
      expect(output.metadata.agent).toBe("einherjar")
    ...
    test("should preserve metadata when appending standalone verification reminder", async () => {
      // given - Heimdall caller without boulder state, metadata containing sessionId
...
session_id: ses_standalone_def
     ...
     test("should still transform when plan is complete (shows progress)", async () => {
       // given - boulder state with complete plan, Heimdall caller
     ...
     test("should not append unrelated current session to boulder state if not already tracked", async () => {
       // given - boulder state without session-append-test, Heimdall caller
     ...
     test("should not append current session when session lookup fails during append decision", async () => {
       // given - boulder state without session-get-failure-test, Heimdall caller, and session lookup failure
     ...
     test("should not duplicate existing session ID", async () => {
       // given - boulder state already has session-dup-test, Heimdall caller
     ...
     test("should include boulder.json path and notepad path in transformed output", async () => {
       // given - boulder state, Heimdall caller
...
`)
      ...
      const beforeHandler = createToolExecuteBeforeHandler({
        ...
        pendingFilePaths,
        pendingTaskRefs,
        isCallerOrchestrator: async (id) => callerAgentBySession.get(id ?? "") === "heimdall",
      })
     ...
     test("should persist preferred subagent session for the current top-level task", async () => {
       // given - boulder state with a current top-level task, Heimdall caller
...
session_id: ses_auth_flow_123
     ...
     const updatedState = readBoulderState(TEST_DIR)
      ...
      expect(updatedState?.task_sessions?.["todo:1"]?.agent).toBe("einherjar")
     ...
     test("should preserve the delegated task key even after the plan advances to the next task", async () => {
       // given - Heimdall caller starts task 1, then the plan advances before task output is processed
...
`)
      ...
      // when - Heimdall delegates task 1
...
session_id: ses_auth_flow_123
     ...
     test("should not overwrite the current task mapping when task() explicitly resumes an older session", async () => {
       // given - current plan is on task 2, but Heimdall explicitly resumes an older session for a previous task
...
`)
      ...
      // when - Heimdall resumes an explicit prior session
      await hook["tool.execute.before"](
...
session_id: ses_old_task_111
      ...
      // then - Heimdall does not poison task 2's preferred session mapping
      const updatedState = readBoulderState(TEST_DIR)
      expect(updatedState?.task_sessions?.["todo:2"]).toBeUndefined()
...
session_id: ses_untrusted_999
    ...
    describe("completion gate output ordering", () => {
      ...
      beforeEach(() => {
        setupMessageStorage(COMPLETION_GATE_SESSION, "heimdall")
      })
    ...
    describe("Write/Edit tool direct work reminder", () => {
      const ORCHESTRATOR_SESSION = "orchestrator-write-test"
       ...
       beforeEach(() => {
         setupMessageStorage(ORCHESTRATOR_SESSION, "heimdall")
       })
      ...
      test("should NOT append reminder when non-orchestrator writes outside .omo/", async () => {
        ...
        setupMessageStorage(nonOrchestratorSession, "einherjar")
  ...
  describe("session.idle handler (boulder continuation)", () => {
    ...
    async function flushMicrotasks(): Promise<void> {
     ...
     beforeEach(() => {
       ...
       registerAgentName("odin")
        ...
        setupMessageStorage(MAIN_SESSION_ID, "heimdall")
      })
    ...
    test("should inject continuation when boulder has incomplete tasks", async () => {
      ...
      const hook = createTestHeimdallHook(mockInput)
    ...
    test("should settle idle before injecting boulder continuation", async () => {
      ...
      const mockInput = createMockPluginInput()
      const hook = createTestHeimdallHook(mockInput, { idleSettleMs: 50 })
    ...
    test("should not append lineage-only subagent session during idle without explicit boulder tracking", async () => {
      ...
      updateSessionAgent(subagentSessionID, "heimdall")
    ...
    test("should inject when registered boulder session has incomplete tasks even if last agent differs", async () => {
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "thor")
    ...
    test("should inject completion nudge when mirrored worktree plan is complete even if the main repo plan is stale", async () => {
      ...
      const worktreeDir = join(tmpdir(), `heimdall-worktree-${randomUUID()}`)
...
`)
    ...
    test("#given continuation emits text without tool progress #when three continuation iterations repeat #then Heimdall stalls instead of looping", async () => {
    ...
    test("#given one plan stalls #when a different boulder plan becomes active #then Heimdall continues the new plan", async () => {
    ...
    test("should inject when last agent is odin and boulder targets heimdall explicitly", async () => {
       // given - boulder explicitly set to heimdall, but last agent is odin (initial state after /start-work)
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // given - last agent is odin (typical state right after /start-work)
       cleanupMessageStorage(MAIN_SESSION_ID)
       setupMessageStorage(MAIN_SESSION_ID, "odin")
       ...
       // then - should call prompt because odin is always allowed for heimdall boulders
       expect(mockInput._promptMock).toHaveBeenCalled()
     })

    test("should inject when registered heimdall boulder session last agent does not match", async () => {
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
     ...
     test("should inject when last agent matches boulder agent even if non-Heimdall", async () => {
       // given - boulder state expects odin and last agent is odin
       const planPath = join(TEST_DIR, "test-plan.md")
       writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
       ...
       // then - should call prompt for odin
       expect(mockInput._promptMock).toHaveBeenCalled()
       const callArgs = mockInput._promptMock.mock.calls[0][0]
       expect(callArgs.body.agent).toBe("odin")
     })
    ...
    test("should preserve display-name agent in continuation prompt when boulder agent uses display form", async () => {
      ...
      expect(mockInput._promptMock).toHaveBeenCalled()
      const callArgs = mockInput._promptMock.mock.calls[0][0]
      expect(callArgs.body.agent).toBe("Heimdall - Plan Executor")
      expect(callArgs.body.agent).not.toBe("heimdall")
    })
    ...
    test("should inject when session agent was updated to heimdall by start-work even if message storage agent differs", async () => {
      // given - boulder targets heimdall, but nearest stored message still says thor
      const planPath = join(TEST_DIR, "test-plan.md")
      writeFileSync(planPath, "# Plan\n- [ ] Task 1\n- [ ] Task 2")
      ...
      cleanupMessageStorage(MAIN_SESSION_ID)
      setupMessageStorage(MAIN_SESSION_ID, "thor")
      updateSessionAgent(MAIN_SESSION_ID, "heimdall")
      ...
      // then - should continue because start-work updated session agent to heimdall
      expect(mockInput._promptMock).toHaveBeenCalled()
    })
