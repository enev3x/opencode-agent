...
type HeimdallHookContext = Parameters<typeof createHeimdallHook>[0]
...
\`\`\`bash
bun test packages/omo-opencode/src/hooks/heimdall/final-wave-approval-gate.test.ts
\`\`\``
...
describe("Heimdall final verification approval gate", () => {
  let testDirectory = ""

  function createMockPluginInput(): HeimdallHookContext & { _promptMock: PromptMock } {
    ...
    Reflect.set(client.session, "get", async ({ path }: { path: { id: string } }) => {
      const parentID = path.id === "ses_final_wave_review"
        ? "heimdall-final-wave-session"
        : path.id === "ses_feature_task"
          ? "heimdall-non-final-session"
          : "main-session-123"
      return {
        data: {
          id: path.id,
    ...
    return {
      ...
      project: {} as HeimdallHookContext["project"],
      worktree: testDirectory,
      serverUrl: new URL("http://localhost"),
      $: {} as HeimdallHookContext["$"],
      client,
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `heimdall-final-wave-test-${randomUUID()}`)
  ...
  test("waits for explicit user approval after the last final-wave approval arrives", async () => {
    // given
    const sessionID = "heimdall-final-wave-session"
...
- [x] F1. **Plan Compliance Audit** - \`volva\`
...
`,
    ...
    const hook = createHeimdallHook(mockInput, { directory: testDirectory, isCallerOrchestrator: async () => true })
    const toolOutput = {
      title: "Odin Task",
...
session_id: ses_final_wave_review
  ...
  test("keeps normal auto-continue instructions for non-final tasks", async () => {
    // given
    const sessionID = "heimdall-non-final-session"
...
## Final Verification Wave (MANDATORY - after ALL implementation tasks)
- [ ] F1. **Plan Compliance Audit** - \`volva\`
...
`,
    ...
    const hook = createHeimdallHook(createMockPluginInput(), {
      directory: testDirectory,
