...
import { handleHeimdallSessionIdle } from "./idle-event"
import type { SessionState } from "./types"
import { unsafeTestValue } from "../../../../../test-support/unsafe-test-value"

describe("handleHeimdallSessionIdle completion nudge", () => {
  const SESSION_ID = "session-main-1"

  let testDirectory = ""

  beforeEach(() => {
    testDirectory = join(tmpdir(), `heimdall-idle-complete-${randomUUID()}`)
  ...
  it("injects BOULDER COMPLETE prompt once per work with substituted elapsed and task breakdown", async () => {
    ...
    const boulder = createBoulderState(planPath, SESSION_ID, "heimdall")
    ...
    // when
    await handleHeimdallSessionIdle({
      ctx,
  ...
  it("#given pending background task #when session idles #then continuation waits for retry instead of prompting", async () => {
    ...
    await handleHeimdallSessionIdle({
      ...
      options: {
        directory: testDirectory,
        backgroundManager: unsafeTestValue<NonNullable<Parameters<typeof handleHeimdallSessionIdle>[0]["options"]>["backgroundManager"]>({
