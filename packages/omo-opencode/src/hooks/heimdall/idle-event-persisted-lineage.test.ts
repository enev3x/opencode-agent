...
const TEST_STORAGE_ROOT = join(tmpdir(), `heimdall-persisted-lineage-storage-${randomUUID()}`)
...
const { createHeimdallHook } = await import("./index")

describe("heimdall hook idle-event persisted lineage", () => {
  ...
  function createHook(
    ...
    return createHeimdallHook(unsafeTestValue<Parameters<typeof createHeimdallHook>[0]>({
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `heimdall-persisted-lineage-${randomUUID()}`)
  ...
  test("does not inject continuation for untracked persisted descendant session without in-memory subagent state", async () => {
    ...
    writeIncompleteBoulder({ agent: "heimdall" })
    ...
    const hook = createHook(
      ...
      {
        [descendantSessionID]: [
          { info: { agent: "heimdall", providerID: "openai", modelID: "gpt-5.4" } },
        ],
      },
    )
  ...
  test("does not inject continuation for persisted appended descendant with mismatched agent", async () => {
    ...
    setSessionAgent(descendantSessionID, "einherjar")
    const hook = createHook(
      {
        [descendantSessionID]: MAIN_SESSION_ID,
      },
      {
        [descendantSessionID]: [
          { info: { agent: "einherjar", providerID: "openai", modelID: "gpt-5.4" } },
        ],
      },
    )
  ...
  test("does not inject continuation for appended descendant when lineage cannot be proven", async () => {
    ...
    const hook = createHeimdallHook(unsafeTestValue<Parameters<typeof createHeimdallHook>[0]>({
      directory: testDirectory,
      client: {
        session: {
          get: async () => {
          ...
          messages: async () => ({
            data: [{ info: { agent: "heimdall", providerID: "openai", modelID: "gpt-5.4" } }],
          }),
          prompt: async (input: unknown) => {
            promptCalls.push(input)
            return { data: {} }
  ...
  test("#given appended descendant lineage and matching agent #when descendant idles #then heimdall injects continuation", async () => {
    // given
  ...
  test("#given appended descendant lineage with odin agent #when heimdall owns boulder #then heimdall still injects continuation", async () => {
    // given
    const descendantSessionID = "ses_appended_descendant_odin"
    writeIncompleteBoulder({
      agent: "heimdall",
      session_ids: [MAIN_SESSION_ID, descendantSessionID],
      session_origins: {
    ...
    setSessionAgent(descendantSessionID, "odin")
    ...
    const hook = createHook(
      ...
      {
        [descendantSessionID]: [
          { info: { agent: "odin", providerID: "openai", modelID: "gpt-5.4" } },
        ],
      },
    )
