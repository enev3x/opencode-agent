...
export async function handleCompletedBoulderIdle(input: {
  ...
  const heimdallAgent = resolveRegisteredAgentName(
    boulderState.agent ?? (isAgentRegistered("heimdall") ? "heimdall" : undefined),
  )
  if (heimdallAgent && isAgentRegistered(heimdallAgent)) {
