...
const TEST_STORAGE_ROOT = join(tmpdir(), `heimdall-session-last-agent-${Date.now()}`)
...
function createTempMessageDir(sessionID: string): string {
  const directory = mkdtempSync(join(tmpdir(), "heimdall-session-last-agent-json-"))
...
describe("getLastAgentFromSession JSON backend", () => {
  ...
  test("skips JSON messages whose part storage contains a compaction marker", async () => {
    ...
    const messageDir = createTempMessageDir(sessionID)
    const compactionMessageID = "msg_test_heimdall_compaction_marker"
  ...
  test("uses SDK messages when JSON message directory lookup fails", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_0001", info: { agent: "odin", time: { created: 100 } } },
            { id: "msg_0002", info: { agent: "heimdall", time: { created: 200 } } },
          ],
        }),
      },
    }
  ...
  test("prefers SDK messages over JSON storage when a client is available", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_0001", info: { agent: "heimdall", time: { created: 100 } } },
          ],
        }),
      },
    }
  ...
  test("prefers top-level SDK agent over conflicting JSON storage when info.agent is absent", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_0001", agent: "Heimdall", info: { time: { created: 100 } } },
          ],
        }),
      },
    }
