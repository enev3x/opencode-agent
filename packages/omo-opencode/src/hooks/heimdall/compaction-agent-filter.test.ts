...
const TEST_STORAGE_ROOT = join(tmpdir(), `heimdall-compaction-storage-${randomUUID()}`)
...
describe("heimdall hook compaction agent filtering", () => {
  ...
  function createMockPluginInput() {
    ...
    } as Parameters<typeof createHeimdallHook>[0] & { _promptMock: ReturnType<typeof mock> }
  }
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `heimdall-compaction-test-${randomUUID()}`)
  ...
  test("should inject continuation when the latest message is compaction but the previous agent matches heimdall", async () => {
    // given
    ...
    writeMessage(sessionID, "msg_001.json", "heimdall")
