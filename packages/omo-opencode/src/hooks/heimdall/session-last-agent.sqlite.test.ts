      ...
      session: {
        messages: async () => ({
          data: [
            ...
            { id: "msg_0002", info: { agent: "einherjar", time: { created: 100 } } },
          ],
        }),
      },
    }
  ...
  test("handles equal timestamps with random-looking ids deterministically", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_a91f00ab", info: { agent: "heimdall", time: { created: 100 } } },
            { id: "msg_f0e1d2c3", info: { agent: "compaction", time: { created: 200 } } },
            { id: "msg_d4c3b2a1", info: { agent: "einherjar", time: { created: 100 } } },
          ],
        }),
      },
    }
  ...
  test("skips compaction marker user messages that retain the original agent", async () => {
    ...
    const client = {
      session: {
        messages: async () => ({
          data: [
            { id: "msg_real", info: { agent: "odin", time: { created: 100 } } },
            {
              id: "msg_compaction",
              info: { agent: "heimdall", time: { created: 200 } },
