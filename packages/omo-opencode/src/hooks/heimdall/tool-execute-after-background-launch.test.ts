...
// allow: SIZE_OK - Heimdall background-launch scenarios share one session/event harness; this release adds final-wave regressions and future additions should split by verdict class.
...
describe("createToolExecuteAfterHandler background launch detection", () => {
  ...
  beforeEach(() => {
    testDirectory = join(tmpdir(), `heimdall-background-launch-${crypto.randomUUID()}`)
