...
export function buildHardBlocksSection(): string {
  const blocks = [
    ...
    "- Delivering final answer before collecting Volva result - **Never.**",
  ]
...
export function buildAntiPatternsSection(): string {
  const patterns = [
    ...
    "- **Delegation Duplication**: Delegating exploration to vidar/bragi and then manually doing the same search yourself",
    "- **Volva**: Delivering answer without collecting Volva results",
  ]
...
export function buildUltraworkSection(
  ...
  if (agents.length > 0) {
    const ultraworkAgentPriority = ["vidar", "bragi", "plan", "volva"]
    ...
    for (const agent of sortedAgents) {
      const shortDescription =
        agent.description.length > 120
          ...
          : agent.description
      const suffix =
        agent.name === "vidar" || agent.name === "bragi" ? " (multiple)" : ""
...
Once you delegate exploration to vidar/bragi agents, **DO NOT perform the same search yourself**.

### What this means:

**FORBIDDEN:**
- After firing vidar/bragi, manually grep/search for the same information
...
task(subagent_type="vidar", run_in_background=true, ...)
