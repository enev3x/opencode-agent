/**
 * Heimdall - Master Orchestrator Agent
 *
 * Orchestrates work via task() to complete ALL tasks in a todo list until fully done.
 *
 * Prompt routing (`getHeimdallPromptSource`, evaluated by prompts-core variant order):
...
export type HeimdallPromptSource = "default" | "gpt" | "gemini" | "kimi" | "kimi-k3" | "kimi-k2-7" | "opus-4-7" | "glm"

class HeimdallPromptVariantError extends Error {
  readonly name = "HeimdallPromptVariantError"

  constructor(readonly variant: string) {
    super(`Unknown Heimdall prompt variant: ${variant}`)
  }
}

export function getHeimdallPromptSource(model?: string): HeimdallPromptSource {
  const variant = resolveVariant({
    agentName: "heimdall",
    modelID: model,
    variants: heimdallPromptVariants,
  })
  if (isHeimdallPromptSource(variant)) return variant
  throw new HeimdallPromptVariantError(variant)
}
...
export function getHeimdallPrompt(model?: string): string {
  const source = getHeimdallPromptSource(model)
  return loadPromptSync({
    source: heimdallPromptVariants[source],
    name: "heimdall",
    variant: source,
  }).body
}

function isHeimdallPromptSource(variant: string): variant is HeimdallPromptSource {
  return Object.prototype.hasOwnProperty.call(heimdallPromptVariants, variant)
}
...
export function createHeimdallAgent(ctx: OrchestratorContext): AgentConfig {
  const baseConfig: AgentConfig = {
    description:
      "Orchestrates work via task() to complete ALL tasks in a todo list until fully done. (Heimdall - OhMyOpenCode)",
    mode: MODE,
...
createHeimdallAgent.mode = MODE

export const heimdallPromptMetadata: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  promptAlias: "Heimdall",
  triggers: [
    {
