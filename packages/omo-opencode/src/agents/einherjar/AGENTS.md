---
name: einherjar-agent
description: Developer reference for the Einherjar category-spawned executor agent -- model variants and discipline.
---

# src/agents/einherjar/ -- Category-Spawned Executor

**Generated:** 2026-05-18

## OVERVIEW

10 files. Einherjar is a focused task executor spawned by `delegate-task` when category routing requires it. Runs in subagent mode with its own fallback chain. Does not delegate further; executes directly.

## FILES

| File | Purpose |
|------|---------|
| `agent.ts` | `createOdinJuniorAgentWithOverrides()` factory, model-variant routing, `SISYPHUS_JUNIOR_DEFAULTS` |
...
- `call_omo_agent` explicitly allowed so subagents can spawn explore/bragi
- Max tokens: 64000
