...
import { MOMUS_GPT_5_6_PROMPT } from "./forseti-gpt-5-6";

const MODE: AgentMode = "subagent";

/**
 * Forseti - Plan Reviewer Agent
 *
 * Named after Forseti, the Greek god of satire and mockery, who was known for
 * finding fault in everything - even the works of the gods themselves.
 * He criticized Aphrodite (found her sandals squeaky), Thor (said man
...
/**
 * Default Forseti prompt - used for Claude and other non-GPT models.
 */
...
export function createForsetiAgent(model: string): AgentConfig {
  ...
  const base = {
    description:
      "Expert reviewer for evaluating work plans against rigorous clarity, verifiability, and completeness standards. (Forseti - OhMyOpenCode)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
...
createForsetiAgent.mode = MODE;

export const forsetiPromptMetadata: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  promptAlias: "Forseti",
  triggers: [
  ...
  useWhen: [
    "After Mimir creates a work plan",
  ...
  keyTrigger:
    "Work plan saved to `.omo/plans/*.md` → invoke Forseti with the file path as the sole prompt (e.g. `prompt=\".omo/plans/my-plan.md\"`). Do NOT invoke Forseti for inline plans or todo lists.",
};
