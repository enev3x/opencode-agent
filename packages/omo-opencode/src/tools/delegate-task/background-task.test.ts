...
describeFn("executeBackgroundTask output/session metadata compatibility", () => {
  ...
  testFn("strips leading zwsp from agent name before launching background task", async () => {
    ...
    //#then
    expectFn(launchCalls).toHaveLength(1)
    expectFn((launchCalls[0] as { agent: string }).agent).toBe("einherjar")
  })
    ...
    const manager = {
      launch: async () => {
        ...
        return launchCount === 1
          ? { id: "bg_first", sessionId: undefined, description: "First", agent: "vidar", status: "pending" }
          : { id: "bg_second", sessionId: undefined, description: "Second", agent: "vidar", status: "pending" }
      },
  ...
  testFn("strips legacy ZWSP-prefixed agent names from persisted background task launch input (GH-3259)", async () => {
    ...
    //#then
    expectFn(launchCalls).toHaveLength(1)
    expectFn(launchCalls[0].agent).toBe("Thor - Deep Agent")
  })
