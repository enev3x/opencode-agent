...
export async function resolveCategoryExecution(
  ...
  const { client, userCategories, odinJuniorModel } = executorCtx
...
Available categories: ${allCategoryNames}`)
  ...
  const overrideModel = odinJuniorModel
  const explicitCategoryModel = userCategories?.[args.category!]?.model

  if (!requirement) {
    // Precedence: explicit category model > einherjar default > category resolved model
    // This keeps `einherjar.model` useful as a global default while allowing
