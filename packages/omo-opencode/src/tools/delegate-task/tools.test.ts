...
describe("odin-task", () => {
  ...
  describe("isPlanAgent", () => {
    ...
    test("returns false for 'mimir' (decoupled from plan)", () => {
      //#given / #when
      const result = isPlanAgent("mimir")

      //#then - mimir is NOT a plan agent
    ...
    test("returns false for case-insensitive match 'Mimir' (decoupled from plan)", () => {
      //#given / #when
      const result = isPlanAgent("Mimir")
    ...
    test("returns false for 'volva'", () => {
      // given / #when
      const result = isPlanAgent("volva")

      // then
      expect(result).toBe(false)
    })

    test("returns false for 'vidar'", () => {
      // given / #when
      const result = isPlanAgent("vidar")
    ...
    test("returns false for non-plan agent display names (regression: isPlanAgent display-name false-positive)", () => {
      //#given / #when / #then
      expect(isPlanAgent(getAgentDisplayName("urd"))).toBe(false)
      expect(isPlanAgent(getAgentDisplayName("forseti"))).toBe(false)
      expect(isPlanAgent(getAgentDisplayName("heimdall"))).toBe(false)
    })
  })
  ...
  describe("isPlanFamily", () => {
    ...
    test("returns true for 'mimir'", () => {
      //#given / #when
      const result = isPlanFamily("mimir")
      //#then
      expect(result).toBe(true)
    })

    test("returns true for mimir display name", () => {
      //#given / #when
      const result = isPlanFamily(getAgentDisplayName("mimir"))
      //#then
      expect(result).toBe(true)
    })

    test("returns true for mimir list display name with zwsp prefix", () => {
      //#given / #when
      const result = isPlanFamily(getAgentListDisplayName("mimir"))
      //#then
      expect(result).toBe(true)
    })
    ...
    test("returns false for 'volva'", () => {
      //#given / #when
      const result = isPlanFamily("volva")
      //#then
      expect(result).toBe(false)
    })

    test("returns false for 'Forseti (Plan Critic)' - no substring false-positive", () => {
      //#given / #when
      const result = isPlanFamily("Forseti (Plan Critic)")
      //#then
      expect(result).toBe(false)
    })
    ...
    test("returns false for 'Urd - Plan Consultant' - no substring false-positive", () => {
      //#given / #when
      const result = isPlanFamily("Urd - Plan Consultant")
      //#then
      expect(result).toBe(false)
    })
    ...
    test("returns false for non-plan-family agent display names (regression: isPlanFamily includes() false-positive)", () => {
      //#given / #when / #then
      expect(isPlanFamily(getAgentDisplayName("urd"))).toBe(false)
      expect(isPlanFamily(getAgentDisplayName("forseti"))).toBe(false)
      expect(isPlanFamily(getAgentDisplayName("heimdall"))).toBe(false)
    })

    test("PLAN_FAMILY_NAMES contains plan and mimir", () => {
      //#given / #when / #then
      expect(PLAN_FAMILY_NAMES).toEqual(["plan", "mimir"])
    })
  })
  ...
  describe("category delegation config validation", () => {
    test("fills subagent_type as einherjar when category is provided without subagent_type", async () => {
      // given
      const { createDelegateTask } = require("./tools")
      ...
      // then
      expect(requireCapturedLaunchInput(launchInput).agent).toBe("Einherjar")
    }, { timeout: 10000 })
    ...
    test("prefers category over subagent_type when both are provided", async () => {
      ...
      //#then - category takes precedence, subagent_type is overridden to einherjar
      expect(requireCapturedLaunchInput(launchInput).agent).toBe("Einherjar")
    }, { timeout: 10000 })
    ...
    test("proceeds without error when systemDefaultModel is undefined", async () => {
      ...
      const { createDelegateTask } = require("./tools")
       ...
       const mockManager = { launch: async () => ({ id: "task-123", status: "pending", description: "Test task", agent: "einherjar", sessionId: "test-session" }) }
       const mockClient = {
         app: { agents: async () => ({ data: [] }) },
  ...
  describe("background metadata sessionId", () => {
    test("should wait for background sessionId and set metadata for TUI toolcall counting", async () => {
      ...
      const mockManager = {
        ...
        launch: async () => {
          const task = { id: "bg_1", status: "pending", description: "Test task", agent: "vidar" }
       ...
       const mockClient = {
         app: { agents: async () => ({ data: [{ name: "vidar", mode: "subagent" }] }) },
         config: { get: async () => ({}) },
         provider: { list: async () => ({ data: { connected: ["openai"] } }) },
         model: { list: async () => ({ data: [{ provider: "openai", id: "gpt-5.5" }] }) },
         session: {
  ...
  describe("skills parameter", () => {
    ...
    test("#given load_skills=null #when executing #then throws (explicit invalid stays rejected, fixes #4119 review)", async () => {
      // given - maintainer's Volva review on PR #4121 (blocker 1) required us
  ...
  describe("run_in_background parameter", () => {
    test("#given category without run_in_background #when executing #then defaults to sync and proceeds (fixes #4119)", async () => {
      ...
      await tool.execute(
        ...
        { sessionID: "parent-session", messageID: "parent-message", agent: "odin", abort: new AbortController().signal }
      )
    ...
    test("#given task_id without run_in_background #when executing #then defaults to sync continuation (fixes #4119)", async () => {
      ...
      // Volva review on PR #4121 asked us to rewrite it (not delete it) so
    ...
    test("#given explicit run_in_background=false #when executing #then sync execution succeeds", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
      const mockClient = {
        app: { agents: async () => ({ data: [{ name: "volva", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-opus-4-7" } }] }) },
        config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
        session: {
          get: async () => ({ data: { directory: "/project" } }),
    ...
    test("#given concurrent background launches from the same parent #when one parent call aborts during session wait #then sibling launch is not interrupted", async () => {
      ...
      const [firstResult, secondResult] = await Promise.all([
        tool.execute(
          ...
          { sessionID: "parent-session", messageID: "parent-message-1", agent: "odin", abort: firstAbortController.signal }
        ),
        tool.execute(
          {
            description: "Tool second",
          ...
          { sessionID: "parent-session", messageID: "parent-message-2", agent: "odin", abort: secondAbortController.signal }
        ),
      ])
  ...
  test("sync continuation preserves variant from previous session message", async () => {
    ...
    const tool = createDelegateTask({
      manager: { resume: async () => ({ id: "task-var", sessionId: "ses_var_test", description: "Variant test", agent: "einherjar", status: "running" }) },
      client: mockClient,
    })
    ...
    expect(callArgs.body.agent).toBe("einherjar")
  ...
  describe("category model resolution fallback", () => {
    test("category uses resolved.model when connectedProvidersCache is null and availableModels is empty", async () => {
      ...
      // NO userCategories override, NO odinJuniorModel
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
    ...
    test("einherjar model override takes precedence over category model", async () => {
      // given - einherjar override model differs from category default
      const { createDelegateTask } = require("./tools")
      let launchInput: CapturedLaunchInput = {}
    ...
    test("explicit category model takes precedence over einherjar model", async () => {
      // given - explicit category model differs from einherjar override
      const { createDelegateTask } = require("./tools")
      let launchInput: CapturedLaunchInput = {}
    ...
    test("einherjar model override works with quick category (#1295)", async () => {
      // given - user configures agents.einherjar.model but uses quick category
      const { createDelegateTask } = require("./tools")
      let launchInput: CapturedLaunchInput = {}
      ...
      // then - einherjar override model should be used, not category default
      expect(launchInput.model.providerID).toBe("anthropic")
      expect(launchInput.model.modelID).toBe("claude-sonnet-4-6")
      expect(launchInput.fallbackChain).toBeUndefined()
    })

    test("einherjar model override works with user-defined category (#1295)", async () => {
      ...
      const tool = createDelegateTask({
        manager: mockManager,
        client: mockClient,
        odinJuniorModel: "openai/gpt-5.5",
        userCategories: {
      ...
      // then - einherjar override model should be used as fallback
      expect(launchInput.model.providerID).toBe("openai")
      expect(launchInput.model.modelID).toBe("gpt-5.5")
    })
  })
  ...
  describe("browserProvider propagation", () => {
    ...
    test("does not prepend plan agent prompt for mimir agent", () => {
      //#given - mimir is NOT a plan agent (decoupled)
      const { buildSystemContent } = require("./tools")
      ...
      //#then - mimir should NOT get plan agent system prepend
      expect(result).toBe(skillContent)
      expect(result).not.toContain("MANDATORY CONTEXT GATHERING PROTOCOL")
    })

    test("does not prepend plan agent prompt for Mimir (case insensitive)", () => {
      //#given - Mimir (capitalized) is NOT a plan agent
      const { buildSystemContent } = require("./tools")
      const skillContent = "You are a strategic planner"
    ...
    test("does not prepend plan agent prompt for non-plan agents", () => {
      ...
      // when
      const result = buildSystemContent({ skillContent, agentName: "volva" })
  ...
  describe("buildTaskPrompt", () => {
    ...
    test("does not append plan guidance for non-plan agents", () => {
      ...
      // when
      const result = buildTaskPrompt(prompt, "vidar")
  ...
  describe("plan family mutual delegation block", () => {
    ...
    test("mimir cannot delegate to plan (cross-blocking)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "mimir", abort: new AbortController().signal }
      )
    ...
    test("mimir display name cannot delegate to plan (cross-blocking)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: getAgentDisplayName("mimir"), abort: new AbortController().signal }
      )
    ...
    test("plan cannot delegate to mimir even when it is exposed as a primary agent", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "mimir", mode: "primary" }] }) },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         session: { get: async () => ({ data: { directory: "/project" } }), create: async () => ({ data: { id: "s" } }), prompt: async () => ({ data: {} }), promptAsync: async () => ({ data: {} }), messages: async () => ({ data: [] }), status: async () => ({ data: {} }) },
       }
       const tool = createDelegateTask({ manager: { launch: async () => ({}) }, client: mockClient })
      ...
      //#when
      const result = await tool.execute(
        { description: "test", prompt: "Execute", subagent_type: "mimir", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "plan", abort: new AbortController().signal }
      )
    ...
    test("odin CAN delegate to plan (not in plan family)", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
      const mockClient = {
         app: { agents: async () => ({ data: [{ name: "plan", mode: "subagent" }] }) },
      ...
      const result = await tool.execute(
        { description: "test", prompt: "Create a plan", subagent_type: "plan", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "odin", abort: new AbortController().signal }
      )
  ...
  describe("subagent_type model extraction (issue #1225)", () => {
    test("background mode passes matched agent model to manager.launch", async () => {
      ...
      const mockManager = {
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "vidar", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-haiku-4-5" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to vidar agent via subagent_type
      await tool.execute(
        {
    ...
    test("sync mode passes matched agent model to prompt request", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "volva", mode: "subagent", model: { providerID: "anthropic", modelID: "claude-opus-4-7" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
         session: {
           get: async () => ({ data: { directory: "/project" } }),
           create: async () => ({ data: { id: "ses_volva_model" } }),
           prompt: promptMock,
           promptAsync: promptMock,
           messages: async () => ({
             data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Consultation done" }] }],
           }),
           status: async () => ({ data: { "ses_volva_model": { type: "idle" } } }),
         },
       }
      ...
      // when - delegating to volva agent via subagent_type in sync mode
      await tool.execute(
        {
          description: "Consult volva",
    ...
    test("agentOverrides model takes priority over matchedAgent.model (#1357)", async () => {
      // given - user configured volva to use a specific model in oh-my-opencode.json
      const { createDelegateTask } = require("./tools")
      let promptBody: CapturedPromptBody = {}
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "volva", mode: "subagent", model: { providerID: "openai", modelID: "gpt-5.5" } },
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to volva via subagent_type with user override
      await tool.execute(
        {
          description: "Consult volva with override",
          prompt: "Review architecture",
          subagent_type: "volva",
          run_in_background: false,
          load_skills: [],
    ...
    test("agentOverrides variant is applied when model is overridden (#1357)", async () => {
      // given - user configured volva with model and variant
      const { createDelegateTask } = require("./tools")
      let promptBody: CapturedPromptBody = {}
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         agentOverrides: {
           volva: { model: "anthropic/claude-opus-4-7", variant: "max" },
         },
       })
      ...
      // when - delegating to volva via subagent_type with variant override
      await tool.execute(
        {
          description: "Consult volva with variant",
          prompt: "Review architecture",
          subagent_type: "volva",
          run_in_background: false,
          load_skills: [],
    ...
    test("fallback chain resolves model when no override and no matchedAgent.model (#1357)", async () => {
      ...
      const mockManager = { launch: async () => ({}) }
       ...
       const mockClient = {
         app: {
           agents: async () => ({
             data: [
               { name: "volva", mode: "subagent" }, // no model field
             ],
           }),
         },
         config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
      ...
      // when - delegating to volva with no override and no matchedAgent model
      await tool.execute(
        {
          description: "Consult volva with fallback",
          prompt: "Review architecture",
          subagent_type: "volva",
          run_in_background: false,
          load_skills: [],
      ...
      // then - should resolve via AGENT_MODEL_REQUIREMENTS fallback chain for volva
      // volva fallback chain: gpt-5.6-sol (openai) > gemini-3.1-pro (google) > claude-opus-4-8 (anthropic)
  ...
  describe("subagent task permission", () => {
    test("plan subagent should have task permission enabled", async () => {
      //#given - odin delegates to plan agent
      const { createDelegateTask } = require("./tools")
      let promptBody: CapturedPromptBody = {}
    ...
    test("mimir primary agent should not be callable via task", async () => {
      //#given
      const { createDelegateTask } = require("./tools")
       const mockClient = {
         app: { agents: async () => ({ data: [{ name: "mimir", mode: "primary" }] }) },
         ...
         session: {
           get: async () => ({ data: { directory: "/project" } }),
           create: async () => ({ data: { id: "ses_mimir_task" } }),
           prompt: async () => ({ data: {} }),
           promptAsync: async () => ({ data: {} }),
           messages: async () => ({ data: [{ info: { role: "assistant" }, parts: [{ type: "text", text: "Plan created" }] }] }),
           status: async () => ({ data: { "ses_mimir_task": { type: "idle" } } }),
         },
       }
       const tool = createDelegateTask({ manager: { launch: async () => ({}) }, client: mockClient })
      ...
      //#when
      const result = await tool.execute(
        { description: "Test mimir task permission", prompt: "Create a plan", subagent_type: "mimir", run_in_background: false, load_skills: [] },
        { sessionID: "p", messageID: "m", agent: "odin", abort: new AbortController().signal }
      )
      
      //#then — coordinator guard fires before primary-agent check; message names the agent and explains the conflict
      expect(result).toContain('Cannot delegate to coordinator agent "mimir" via task()')
    }, { timeout: 20000 })

    test("non-plan subagent should NOT have task permission", async () => {
      //#given - odin delegates to volva (non-plan)
      const { createDelegateTask } = require("./tools")
      let promptBody: CapturedPromptBody = {}
      
      const mockManager = { launch: async () => ({}) }
      const mockClient = {
        app: { agents: async () => ({ data: [{ name: "volva", mode: "subagent" }] }) },
        config: { get: async () => ({ data: { model: SYSTEM_DEFAULT_MODEL } }) },
        session: {
          get: async () => ({ data: { directory: "/project" } }),
          create: async () => ({ data: { id: "ses_volva_no_delegate" } }),
          ...
          status: async () => ({ data: { "ses_volva_no_delegate": { type: "idle" } } }),
        },
      }
      ...
      // when - odin delegates to volva
      await tool.execute(
        {
          description: "Test volva no task permission",
      ...
      // then - volva should NOT have task permission
  ...
  describe("session title and metadata format (OpenCode compatibility)", () => {
    test("sync session title follows OpenCode format: '{description} (@{agent} subagent)'", async () => {
      ...
      expect(createBody.title).toBe("Implement feature X (@Einherjar subagent)")
    }, { timeout: 10000 })
    ...
    test("background task output includes <task_metadata> block with session_id", async () => {
      ...
      const mockManager = {
       ...
       const tool = createDelegateTask({
         ...
         client: mockClient,
         userCategories: {
           "einherjar": { model: "anthropic/claude-sonnet-4-6" },
         },
       })
...
describe("buildSyncPromptTools (issue #5182)", () => {
  test("merges user permission denials into tools (fix for issue #5182)", () => {
    // #given - user configured agents.einherjar.permission with tool denials
    ...
    const result = buildSyncPromptTools("einherjar", userPermission)
  ...
  test("categories.<name>.tools propagates to session tools end-to-end (issue #5182)", async () => {
    ...
    // hardcoded restriction (task: false for einherjar) still applies
