...
describe("executeBackground", () => {
  ...
  test("sanitizes subagent_type before passing to background manager launch", async () => {
    ...
    expect(launchArgs.agent).toBe("Thor - Deep Agent")
  })
  ...
  test("#given subagent_type is the lowercase config key 'thor' #when executeBackground runs #then BackgroundManager.launch receives the registered display name", async () => {
    //#given
    ...
    //#when
    await executeBackground({ ...testArgs, subagent_type: "thor" }, testContext, mockManager, mockClient)
  ...
  test("#given subagent_type is a same-keyed agent 'vidar' #when executeBackground runs #then BackgroundManager.launch receives the unchanged key (regression guard)", async () => {
    //#given
    launchMock.mockClear()
    launchMock.mockResolvedValueOnce({
      id: "test-task-id",
    ...
    //#when
    await executeBackground({ ...testArgs, subagent_type: "vidar" }, testContext, mockManager, mockClient)
    ...
    const launchArgs = latestCall[0]
    if (!launchArgs) throw new Error("Expected launch arguments")
    expect(launchArgs.agent).toBe("vidar")
  })
