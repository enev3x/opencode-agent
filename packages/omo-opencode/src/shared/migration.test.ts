...
describe("migrateAgentNames", () => {
  test("migrates legacy OmO names to lowercase", () => {
    ...
    expect(migrated["odin"]).toEqual({ temperature: 0.5 })
    expect(migrated["mimir"]).toEqual({ prompt: "custom prompt" })
  ...
  test("preserves current agent names unchanged", () => {
    ...
    const agents = {
      volva: { model: "openai/gpt-5.5-preview" },
    ...
    expect(migrated["volva"]).toEqual({ model: "openai/gpt-5.5-preview" })
    expect(migrated["bragi"]).toEqual({ model: "google/gemini-3-flash" })
    expect(migrated["vidar"]).toEqual({ model: "opencode/gpt-5-nano" })
  })
  ...
  test("handles case-insensitive migration", () => {
    ...
    const agents = {
      SISYPHUS: { model: "test" },
      "planner-odin": { prompt: "test" },
      "Orchestrator-Odin": { model: "openai/gpt-5.4" },
    }
    ...
    expect(migrated["odin"]).toEqual({ model: "test" })
    expect(migrated["mimir"]).toEqual({ prompt: "test" })
    expect(migrated["heimdall"]).toEqual({ model: "openai/gpt-5.4" })
  })
  ...
  test("migrates orchestrator-odin to heimdall", () => {
    // given: Config with legacy orchestrator-odin agent name
    const agents = {
      "orchestrator-odin": { model: "anthropic/claude-opus-4-7" },
    }

    // when: Migrate agent names
    const { migrated, changed } = migrateAgentNames(agents)

    // then: orchestrator-odin should be migrated to heimdall
    expect(changed).toBe(true)
    expect(migrated["heimdall"]).toEqual({ model: "anthropic/claude-opus-4-7" })
    expect(migrated["orchestrator-odin"]).toBeUndefined()
  })

  test("migrates lowercase heimdall to heimdall", () => {
    // given: Config with lowercase heimdall agent name
    const agents = {
    ...
    // then: lowercase heimdall should remain heimdall (no change needed)
    expect(changed).toBe(false)
    expect(migrated["heimdall"]).toEqual({ model: "anthropic/claude-opus-4-7" })
  })

  test("migrates Odin variants to lowercase", () => {
    // given agents config with "Odin" key
    // when migrateAgentNames called
    // then key becomes "odin"
    const agents = { "Odin": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["odin"]).toEqual({ model: "test" })
    expect(migrated["Odin"]).toBeUndefined()
  })

  test("migrates omo key to odin", () => {
  ...
  test("migrates Heimdall variants to lowercase", () => {
    // given agents config with "Heimdall" key
    // when migrateAgentNames called
    // then key becomes "heimdall"
    const agents = { "Heimdall": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["heimdall"]).toEqual({ model: "test" })
    expect(migrated["Heimdall"]).toBeUndefined()
  })

  test("migrates Mimir variants to lowercase", () => {
    // given agents config with "Mimir - Plan Builder" key
    // when migrateAgentNames called
    // then key becomes "mimir"
    const agents = { "Mimir - Plan Builder": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["mimir"]).toEqual({ model: "test" })
    expect(migrated["Mimir - Plan Builder"]).toBeUndefined()
  })

  test("migrates Urd variants to lowercase", () => {
    // given agents config with "Urd - Plan Consultant" key
    // when migrateAgentNames called
    // then key becomes "urd"
    const agents = { "Urd - Plan Consultant": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["urd"]).toEqual({ model: "test" })
    expect(migrated["Urd - Plan Consultant"]).toBeUndefined()
  })

  test("migrates Forseti variants to lowercase", () => {
    // given agents config with "Forseti - Plan Critic" key
    // when migrateAgentNames called
    // then key becomes "forseti"
    const agents = { "Forseti - Plan Critic": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["forseti"]).toEqual({ model: "test" })
    expect(migrated["Forseti - Plan Critic"]).toBeUndefined()
  })

  test("migrates Einherjar to lowercase", () => {
    // given agents config with "Einherjar" key
    // when migrateAgentNames called
    // then key becomes "einherjar"
    const agents = { "Einherjar": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(true)
    expect(migrated["einherjar"]).toEqual({ model: "test" })
    expect(migrated["Einherjar"]).toBeUndefined()
  })

  test("preserves lowercase passthrough", () => {
    // given agents config with "volva" key
    // when migrateAgentNames called
    // then key remains "volva" (no change needed)
    const agents = { "volva": { model: "test" } }
    const { migrated, changed } = migrateAgentNames(agents)
    expect(changed).toBe(false)
    expect(migrated["volva"]).toEqual({ model: "test" })
  })
})
...
describe("migrateHookNames", () => {
  ...
  test("migrates odin-orchestrator to heimdall", () => {
    // given: Config with legacy odin-orchestrator hook
    const hooks = ["odin-orchestrator", "comment-checker"]

    // when: Migrate hook names
    const { migrated, changed, removed } = migrateHookNames(hooks)

    // then: odin-orchestrator should be migrated to heimdall
    expect(changed).toBe(true)
    expect(migrated).toContain("heimdall")
    expect(migrated).toContain("comment-checker")
    expect(migrated).not.toContain("odin-orchestrator")
    expect(removed).toEqual([])
  })
  ...
  test("handles mixed migration and removal", () => {
    ...
    const hooks = ["anthropic-auto-compact", "preemptive-compaction", "odin-orchestrator"]
...
describe("migrateConfigFile", () => {
  ...
  test("migrates omo_agent to odin_agent", () => {
    ...
    // then: omo_agent should be migrated to odin_agent
    expect(needsWrite).toBe(true)
    expect(rawConfig.odin_agent).toEqual({ disabled: false })
  ...
  test("does not write if no migration needed", () => {
   ...
   test("handles migration of all legacy items together", () => {
     ...
     const agents = rawConfig.agents as Record<string, unknown>
     expect(agents["odin"]).toBeDefined()
     expect(agents["mimir"]).toBeDefined()
     expect(rawConfig.disabled_hooks).toContain("anthropic-context-window-limit-recovery")
   })
   ...
   test("does not migrate gpt-5.4-codex model versions in agents", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         odin: { model: "openai/gpt-5.4-codex", temperature: 0.1 },
       },
     }
     ...
     expect(agents["odin"].model).toBe("openai/gpt-5.4-codex")
   })
   ...
   test("does not set needsWrite when no model versions need migration", () => {
     ...
     const rawConfig: Record<string, unknown> = {
       agents: {
         odin: { model: "openai/gpt-5.4-codex" },
       },
       categories: {
...
describe("migration maps", () => {
  test("AGENT_NAME_MAP contains all expected legacy mappings", () => {
    ...
    expect(AGENT_NAME_MAP["omo"]).toBe("odin")
    expect(AGENT_NAME_MAP["OmO"]).toBe("odin")
    expect(AGENT_NAME_MAP["OmO-Plan"]).toBe("mimir")
    expect(AGENT_NAME_MAP["omo-plan"]).toBe("mimir")
    expect(AGENT_NAME_MAP["Planner-Odin"]).toBe("mimir")
    expect(AGENT_NAME_MAP["plan-consultant"]).toBe("urd")
  })
...
describe("migrateModelVersions", () => {
  test("#given a config with gpt-5.4-codex model #when migrating model versions #then does not overwrite with gpt-5.5", () => {
    ...
    expect(changed).toBe(false)
    const odin = migrated["odin"] as Record<string, unknown>
    expect(odin.model).toBe("openai/gpt-5.4-codex")
    expect(odin.temperature).toBe(0.1)
  })
  ...
  test("#given explicit gpt-5.5 pins on current entries (#3777) #when migrating #then preserves every user choice", () => {
    ...
    const configs = {
      thor: { model: "openai/gpt-5.5", variant: "medium" },
      forseti: { model: "openai/gpt-5.5", variant: "xhigh" },
  ...
  test("replaces anthropic model version", () => {
    ...
    expect(changed).toBe(true)
    const mimir = migrated["mimir"] as Record<string, unknown>
    expect(mimir.model).toBe("anthropic/claude-opus-4-8")
  })

  test("leaves unknown model strings untouched", () => {
    // given: Agent config with unknown model
    const agents = {
      volva: { model: "openai/gpt-5.5-preview", temperature: 0.5 },
    }
    ...
    expect(changed).toBe(false)
    const volva = migrated["volva"] as Record<string, unknown>
    expect(volva.model).toBe("openai/gpt-5.5-preview")
  })

  test("handles agent config with no model field", () => {
    // given: Agent config without model field
    const agents = {
      odin: { temperature: 0.1, prompt: "custom" },
    }
  ...
  test("handles agent config with non-string model", () => {
    ...
    const agents = {
      odin: { model: 123, temperature: 0.1 },
    }
  ...
  test("migrates multiple agents in one pass", () => {
    ...
    expect(changed).toBe(true)
    expect((migrated["odin"] as Record<string, unknown>).model).toBe("openai/gpt-5.4-codex")
    expect((migrated["mimir"] as Record<string, unknown>).model).toBe("anthropic/claude-opus-4-8")
    expect((migrated["volva"] as Record<string, unknown>).model).toBe("openai/gpt-5.5-preview")
  })
  ...
  test("handles mixed: some applied, some new", () => {
    ...
    // then: Only mimir should be migrated
    expect(changed).toBe(true)
...
describe("migrateConfigFile _migrations tracking", () => {
  ...
  test("skips re-migration when _migrations contains the key", () => {
    ...
    const odin = (rawConfig.agents as Record<string, Record<string, unknown>>).odin
    expect(odin.model).toBe("openai/gpt-5.4-codex")
  ...
  test("migrates legacy in-config _migrations into the sidecar and appends new migrations (#3263)", () => {
    ...
    expect(rawConfig._migrations).toBeUndefined()
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).mimir.model).toBe("anthropic/claude-opus-4-8")
...
describe("migrateConfigFile with backup", () => {
  ...
  test("preserves model setting without auto-conversion to category", () => {
    ...
    const rawConfig: Record<string, unknown> = {
      agents: {
        "huginn": { model: "anthropic/claude-haiku-4-5" },
        volva: { model: "openai/gpt-5.5-preview" },
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>
    expect(agents["huginn"].model).toBe("anthropic/claude-haiku-4-5")
    expect(agents.volva.model).toBe("openai/gpt-5.5-preview")
  ...
  test("preserves category setting when explicitly set", () => {
    ...
    const agents = rawConfig.agents as Record<string, Record<string, unknown>>
    expect(agents["huginn"].category).toBe("quick")
    expect(agents.volva.category).toBe("ultrabrain")
  })
  ...
  test("does not write when no migration needed", () => {
     ...
     fs.writeFileSync(testConfigPath, globalThis.JSON.stringify({ agents: { odin: { model: "test" } } }, null, 2))
     cleanupPaths.push(testConfigPath)
...
describe("migrateModelVersions with applied migrations", () => {
  ...
  test("handles mixed: skip applied, apply new", () => {
    ...
    const configs = {
      odin: { model: "openai/gpt-5.4-codex" },
      volva: { model: "anthropic/claude-opus-4-4" },
    }
    const appliedMigrations = new Set(["model-version:openai/gpt-5.4-codex->openai/gpt-5.5"])
    ...
    // then: Skip odin (already applied), apply volva
    expect(changed).toBe(true)
    expect(newMigrations).toEqual(["model-version:anthropic/claude-opus-4-4->anthropic/claude-opus-4-8"])
    expect((migrated.odin as Record<string, unknown>).model).toBe("openai/gpt-5.4-codex")
    expect((migrated.volva as Record<string, unknown>).model).toBe("anthropic/claude-opus-4-8")
  })
...
describe("migrateConfigFile with migration tracking via sidecar (#3263)", () => {
  ...
  test("does not emit migration history when no migration applies", () => {
    ...
    expect(needsWrite).toBe(false)
    expect(rawConfig._migrations).toBeUndefined()
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).odin.model).toBe("openai/gpt-5.4-codex")
  ...
  test("writes applied migrations to sidecar instead of leaving them on the config", () => {
    ...
    expect(needsWrite).toBe(true)
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).volva.model).toBe("anthropic/claude-opus-4-8")
    expect(rawConfig._migrations).toBeUndefined()
  ...
  test("skips re-applying a migration that is recorded in the sidecar even if the user edited _migrations away", () => {
    ...
    expect(needsWrite).toBe(false)
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).volva.model).toBe("openai/gpt-5.4")
    expect(rawConfig._migrations).toBeUndefined()
  })
  ...
  test("unions sidecar and legacy _migrations entries, deduplicating", () => {
    ...
    expect((rawConfig.agents as Record<string, Record<string, unknown>>).volva.model).toBe("anthropic/claude-opus-4-4")
