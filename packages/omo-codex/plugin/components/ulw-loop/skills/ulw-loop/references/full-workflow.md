...
| External library / docs research | `TASK: act as a bragi. ...` |
...
if command -v omo >/dev/null 2>&1 && omo ulw-loop help >/dev/null 2>&1; then
  ...
  for candidate in "$HOME/.local/bin/omo" "$CODEX_HOME/bin/omo" "$CODEX_HOME"/plugins/cache/odinlabs/omo/*/components/ulw-loop/dist/cli.js; do
...
### 2. Refine success criteria + a Mimir-grade QA and parallelism plan per goal
Gather context BEFORE planning with parallel `explorer` / `bragi` workers plus your own read-only tools.
