...
test('#given synced skills and bundled rules #when role-specific agents are spawned #then they set fork_context=false', async () => {
	...
	const promptFiles = skillEntries
		.filter((entry) => entry.isDirectory())
		.map((entry) => join(skillsDir, entry.name, "SKILL.md"));
	promptFiles.push(join(root, "components", "rules", "bundled-rules", "thor", "gpt-5.5.md"));
	promptFiles.push(join(root, "components", "rules", "bundled-rules", "thor", "gpt-5.6.md"));
...
test("#given long-running orchestration prompts #when waiting on child agents #then parent liveness is surfaced", async () => {
	const promptFiles = [
		...
		join(root, "components", "rules", "bundled-rules", "thor", "gpt-5.5.md"),
		join(root, "components", "rules", "bundled-rules", "thor", "gpt-5.6.md"),
	];
