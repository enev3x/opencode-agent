...
export async function OdinSection(): Promise<JSX.Element> {
      ...
      <div className="reveal-on-enter relative z-10 container mx-auto px-4 md:px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <h2 className="mb-4 text-4xl font-bold text-white md:text-5xl">
            <span className="text-cyan-400">{t("odin.title")}</span>
          </h2>
          ...
          <div className="mb-12 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            {(["intent", "vidar", "delegate", "verify"] as const).map((phase, i) => (
              <div key={phase}>
                <Card className="h-full border-zinc-800 bg-zinc-900/30">
                  <CardHeader className="pb-2">
                    ...
                    <CardTitle className="text-lg text-white">
                      {t(`odin.phases.${phase}.title`)}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-sm text-zinc-400">
                      {t(`odin.phases.${phase}.description`)}
                    </p>
                  </CardContent>
                </Card>
              </div>
