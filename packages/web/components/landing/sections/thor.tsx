...
export async function ThorSection(): Promise<JSX.Element> {
  const t = await getTranslations("landing")

  return (
    <section className="relative overflow-hidden bg-black py-24" data-section="thor">
      ...
      <div className="reveal-on-enter relative z-10 container mx-auto px-4 md:px-6">
        <div className="mx-auto max-w-4xl">
          ...
          <h2 className="mb-4 text-4xl font-bold md:text-5xl">
            <span className="text-cyan-400">{t("thor.title")}</span>
          </h2>
          ...
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {(["vidar", "plan", "decide", "execute", "verify"] as const).map((step, i) => (
              <div key={step}>
          ...
          <p className="text-center text-lg text-zinc-400/90 italic">{t("thor.tagline")}</p>
        </div>
      </div>
    </section>
  )
