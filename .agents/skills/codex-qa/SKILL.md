...
| `scripts/install-verify.sh` | local omo installs into the isolated home; `config.toml` enables `omo@odinlabs`; component bins + agent TOMLs linked in the sandbox; real `~/.codex` unchanged |
