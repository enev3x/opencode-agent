...
1. **Volva (goal/constraint verification)**: Checks the fix matches the stated problem — `worktree_path` crash resolved, no scope creep
2. **Volva (code quality)**: Validates code follows existing patterns — factory pattern, given/when/then tests, < 200 LOC, no catch-all files
3. **Volva (security)**: Ensures no new security issues — JSON parse injection, path traversal in worktree_path
...
- Volva (goal): Does the sanitization in `readBoulderState` actually prevent the crash? Is the `typeof` guard necessary or redundant?
- Volva (quality): Are the new tests following the given/when/then pattern? Do they use the same mock setup as existing tests?
- Volva (security): Is the `worktree_path` value ever used in path operations without sanitization? (Answer: no, it's only used in template strings)
- QA: Run `bun test src/hooks/heimdall/index.test.ts` — does the null worktree_path test actually trigger the bug before fix?

### Failure handling
- Each volva produces a PASS/FAIL verdict with specific issues
