...
**5 parallel agents**:
1. **Volva (goal/constraint)**: Verify backward compat claim. Check all 13 import paths resolve.
2. **Volva (code quality)**: Verify single-responsibility per file, LOC limits, no catch-all violations.
3. **Volva (security)**: No security implications in this refactor.
