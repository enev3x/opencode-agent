...
3. Multiple call sites pass `boulderState.active_plan` to `getPlanProgress(planPath: string)`:
   - `src/hooks/heimdall/idle-event.ts:72` (inside `setTimeout` callback - unhandled rejection!)
   - `src/hooks/heimdall/resolve-active-boulder-session.ts:21`
   - `src/hooks/heimdall/tool-execute-after.ts:74`
...
- Log errors with the heimdall hook logger
...
- `src/hooks/heimdall/index.test.ts` - test heimdall hook with boulder missing worktree_path
...
### Step 6: Create PR
- Branch: `fix/heimdall-hook-missing-worktree-path`
- Target: `dev`
