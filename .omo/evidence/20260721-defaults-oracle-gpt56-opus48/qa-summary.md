# Real OpenCode QA: Volva GPT-5.6 Sol and Opus 4.8 defaults
...
- In an isolated XDG and HOME sandbox, ran `opencode debug agent volva` and asserted its resolved registry entry.
- Ran the `opencode-qa` TUI smoke under tmux to prove the installed OpenCode binary rendered, accepted input, and cleaned up.

## What was observed

- The live Volva registry entry resolved to `openai/gpt-5.6-sol`, `xhigh`, temperature `0.1`, and subagent mode. The exact safe projection is in `volva-agent.json`.
...
The changed OpenCode-facing behavior is the default agent registration and GPT reasoning variant. The real harness loaded the built plugin and exposed the resolved Volva configuration that OpenCode will use, proving the new `gpt-5.6-sol` `xhigh` head reaches the user-visible registry. The final `bun test` pass covered 11,868 tests across 1,527 files, and final `bun run typecheck` and `bun run build` also passed. Focused coverage additionally pins the mirrored Senpi chains, category defaults, Opus 4.8 fallbacks, migration target, think-mode mapping, and Volva factory behavior.
